...
async def get_pipeline_stats(
    ...
    # Parse statistics
    parsed_successfully = db.query(func.count(Candidate.id)).filter(
        Candidate.parse_status == "success"
    ).scalar()
    
    parse_failed = db.query(func.count(Candidate.id)).filter(
    ...
    avg_scores = db.query(
        ...
        CandidateScore, Job.id == CandidateScore.job_id
    ).group_by(
        Job.id, Job.title
    ).all()
